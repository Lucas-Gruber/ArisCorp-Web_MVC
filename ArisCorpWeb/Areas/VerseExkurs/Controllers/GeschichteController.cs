    ...
    public class GeschichteController : Controller
    {
        ...
        public async Task<ActionResult> Details(string artikel)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                HttpResponseMessage Res = await client.GetAsync("items/geschichte" + "?filter[geschichte_titel]=" + artikel + "&filter[status]=published&access_token=ihGAYzxCs1LWxIGBSTWbx8w3cd7oTNCobhZdmr");
        ...
        [Route("VerseExkurs/Geschichte/Ein-Tag-in-der-Geschichte")]
        public async Task<ActionResult> einTag()
        {
            GeschichteRootobject GeschichteInfo = new GeschichteRootobject();
            ...
            using (var client = new HttpClient())
            {
                ...
                //Sending request to find web api REST service resource GetAllEmployees using HttpClient  
                HttpResponseMessage Res = await client.GetAsync("items/geschichte" + "?filter[status]=published&filter[geschichte_kategorie]=true&sort=geschichte_datum&access_token=ihGAYzxCs1LWxIGBSTWbx8w3cd7oTNCobhZdmr");
